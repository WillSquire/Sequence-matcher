...
pub struct SequenceDetector<'a> { // <'a> is a bit like generics in C#, it pass in the lifetime on struct initialisation (I *think*)
	pub sequence: &'a[u8], // is the array of elements to find in sequence
	pub match_count: u8 // Unsigned as cannot be negative
}

impl<'a> SequenceDetector<'a> {
	//----------------------------------------------------
	// Constructors
	//----------------------------------------------------
    /// Instantiates a new SequenceDetector object from a given array of elements
	pub fn new(sequence: &[u8]) -> SequenceDetector {
